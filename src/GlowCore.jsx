...
export default function GlowCore({
  ...
  const RING_CONFIGS = [
    { type: 'ring', radius: RING_AVG_RADIUS + 0.3, tube: 0.1, opacity: 0.4, speed: 0.5, phase: 0 },
    { type: 'ring', radius: RING_AVG_RADIUS + 0.5, tube: 0.1, opacity: 0.25, speed: 0.7, phase: 1.0 },
    { type: 'ring', radius: RING_AVG_RADIUS + 0.7, tube: 0.1, opacity: 0.15, speed: 0.3, phase: 2.0 }
  ];
  ...
  useFrame((state) => {
    ...
    ringGroupRefs.current.forEach((ref, index) => {
      if (ref) {
        // Different rotation axes and directions for each ring system
        const rotationSpeed = 0.2 + (index * 0.1); // Varies from 0.2 to 0.7
        const direction = index % 2 === 0 ? 1 : -1;

        // Rotate on different axes based on index
        switch (index) {
          case 0: // X-axis rotation
            ref.rotation.x = state.clock.elapsedTime * rotationSpeed * direction;
            break;
          case 1: // Y-axis rotation
            ref.rotation.y = state.clock.elapsedTime * rotationSpeed * direction;
            break;
          case 2: // Z-axis rotation
            ref.rotation.z = state.clock.elapsedTime * rotationSpeed * direction;
            break;
          case 3: // X and Y combined
            ref.rotation.x = state.clock.elapsedTime * rotationSpeed * direction;
            ref.rotation.y = state.clock.elapsedTime * rotationSpeed * 0.5 * direction;
            break;
          case 4: // Y and Z combined
            ref.rotation.y = state.clock.elapsedTime * rotationSpeed * direction;
            ref.rotation.z = state.clock.elapsedTime * rotationSpeed * 0.7 * direction;
            break;
          case 5: // X and Z combined
            ref.rotation.x = state.clock.elapsedTime * rotationSpeed * 0.6 * direction;
            ref.rotation.z = state.clock.elapsedTime * rotationSpeed * direction;
            break;
        }
      }
